...
def train_step(experience, steps):
    global policy_losses, value_losses
...
def train():
    ...
    for episode in range(episodes):
        for rand_prompts in prompts_dataloader:
            ...
            torch.cuda.empty_cache()
    actor_model.save_pretrained("./trained_actor_model")  # 策略模型
    torch.save(optimizer_actor.state_dict(), "./optimizer_actor.pth")
    # 修复 Critic 保存：
    torch.save(critic_model.state_dict(), "./trained_critic_model.pth")  # 使用 torch.save
    actor_tokenizer.save_pretrained("./trained_actor_model")
    # 绘制并保存损失曲线
...
if __name__ == "__main__":
    ...
    # 策略模型
    actor_model = AutoModelForCausalLM.from_pretrained('IDEA-CCNL/Randeng-BART-139M').to(device)
    # 参考模型
    ref_model = AutoModelForCausalLM.from_pretrained('IDEA-CCNL/Randeng-BART-139M').to(device)
    # 奖励模型
    reward_model = AutoModelForSequenceClassification.from_pretrained('distilbert/distilbert-base-cased').to(device)
    actor_tokenizer = AutoTokenizer.from_pretrained('IDEA-CCNL/Randeng-BART-139M')
    reward_tokenizer = AutoTokenizer.from_pretrained('distilbert/distilbert-base-cased')
    # 价值模型
    ...
    prompts_dataset = PromptDataset(prompt_list, actor_tokenizer, apply_chat_template=False)
